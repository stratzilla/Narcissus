...
function shovel(cid, message, keywords, parameters, node)
   if(not npcHandler:isFocused(cid)) then
        return false
    end
    ...
    local player_money     = player_gold + player_plat + player_crys
        ...
        if doPlayerRemoveMoney(cid,50) then
            npcHandler:say('Enjoy your brand new shovel!', cid)
             doPlayerAddItem(cid, 2554, 1)
        end
        else
            npcHandler:say('You don\'t have enough money for this fine piece of equipment!', cid)
    end
end	
...
node1:addChildKeyword({'no'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Alright then. Come back when you have the neccessary items.', reset = true})
...
node2:addChildKeyword({'no'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Alright then. Come back when you have the neccessary items.', reset = true})
...
node3:addChildKeyword({'no'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Alright then. Come back when you have the neccessary items.', reset = true})

node4 = keywordHandler:addKeyword({'help'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'I have stolen something from Dermot. You can buy it if you spare me 800 gold. Deal?'})
node4:addChildKeyword({'yes'}, key, {})
node4:addChildKeyword({'no'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Hmm, maybe next time.', reset = true})

node5 = keywordHandler:addKeyword({'shovel'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Do you want to buy a shovel for 50 gold?'})
node5:addChildKeyword({'yes'}, shovel, {})
node5:addChildKeyword({'no'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Hmm, maybe next time.', reset = true})
