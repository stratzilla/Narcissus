function onUse(cid, item, fromPosition, itemEx, toPosition)
	local REACTION =
	{
		...
		[{8976, 9003}] = {"Flap Flap", "Hail TibiaBR"}, --golden falcon
		[{2816}] = {"Click!", "You don't seem experienced enough to open this properly.", "The box won't open for you.", "This item is too precious for a newbie. You are unable to open it."}, --tibiora's box
	...
	for k, v in pairs(REACTION) do
		...
		elseif (isInArray({2816}, item.itemid)) then
			doTransformItem(item.uid, 2829)
			doDecayItem(item.uid)
