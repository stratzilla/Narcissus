...
function onStepIn(cid, item, position, fromPosition)
	...
	if(vocation >= 0 and vocation < 50 and item.actionid ~= 100) then
	...
	if(item.actionid ~= 0 and item.actionid ~= 100 and getPlayerStorageValue(cid, item.actionid) <= 0) then
		pushBack(cid, position, fromPosition, true)
		return true
	end
	...
	if(getTileInfo(position).protection) then
		...
		if(not isPlayerGhost(cid)) then
			local rockPos = {x=position.x,y=position.y,z=position.z,stackpos=0}
			doCreateItem(1548, 1, rockPos)
		end
		while(true) do
			...
			if(isInArray(depots, depot.itemid)) then
				local depotItems = getPlayerDepotItems(cid, getDepotId(depot.uid)) - 1
					if depotItems == 0 then
						doPlayerSendTextMessage(cid, MESSAGE_STATUS_DEFAULT, "Your depot is empty.")
						break
					end
...
function onStepOut(cid, item, position, fromPosition)
	...
	local asdf = {x=position.x,y=position.y,z=position.z,stackpos=0} --143 to 146 cleans pedestals
	if item.actionid == 100 or item.actionid == 0 then
		doCleanTile(asdf)
	end
